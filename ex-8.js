...
function totalPaidByLocation(bills) {
  // ใช้ reduce เพื่อรวมผลรวม total ตาม location
  const totalsByLocation = bills.reduce((acc, cur) => {
    if (acc[cur.location]) {
      // ตรวจสอบว่า location ปัจจุบันมีอยู่ใน accumulator (acc) หรือไม่
      acc[cur.location] += cur.total; // ถ้ามีอยู่แล้ว, เพิ่ม total ของบิลปัจจุบันเข้าไปในค่าเดิม
    } else {
      acc[cur.location] = cur.total; // ถ้ายังไม่มี สร้างคีย์ใหม่ใน acc และตั้งค่ามันให้เป็น total ของบิลปัจจุบัน
    }
    //console.log(acc);
    return acc; // คืนค่า acc เพื่อให้มันเป็นค่า accumulator ในการวนลูปครั้งถัดไป
  }, {});
