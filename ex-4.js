...
// Start coding here
const newBills = bills.filter(({ member }) => member != null);

const billMembers = newBills.map(({ member }) => member.name);

const totalMembers = billMembers.reduce(
  (acc, name, index) => (billMembers.indexOf(name) === index ? acc + 1 : acc),
  0
);
